...
class VoteControls extends Component {
  ...
  async getBoxVoteCredits(address, creditsToLock) {
    ...
    const selectedInputs = Tx.calcInputs(
      allCreditUtxos, address, creditsToLock.abs().toString(), parseInt(VOICE_CREDITS_COLOR, 10), 8
    );
  ...
  async getVoteTokens(address, amount) {
    ...
    const selectedInputs = Tx.calcInputs(
      allVoteTokensUTXOs, address, amount.abs().toString(), parseInt(VOICE_TOKENS_COLOR, 10), 8
    );
