...
import { Input, QR as QRCode, Text, Select, Flex, Checkbox } from "rimble-ui";
...
export default class Advanced extends React.Component {
  ...
  componentDidMount() {
    ...
    const currency =
      getStoredValue("currency", address) || CURRENCY.DEFAULT_CURRENCY;
    ...
    if (this.state.newPrivateKey) {
      inputPrivateEyeButton = (
        <PrimaryButton
          ...
          onClick={() => {
            this.setState({ privateKeyHidden: !this.state.privateKeyHidden });
          }}
        >
      ...
      <div className="content ops row settings-row">
        <div className="input-with-toggle">
          <Input
            ...
            onChange={event =>
              this.setState({ newPrivateKey: event.target.value })
            }
          />
          {inputPrivateEyeButton}
        </div>
    ...
    if (this.state.newSeedPhrase) {
      inputSeedEyeButton = (
        <PrimaryButton
          width={1}
          onClick={() => {
            this.setState({ seedPhraseHidden: !this.state.seedPhraseHidden });
          }}
        >
          <i className="fas fa-eye" />
        </PrimaryButton>
    ...
    let inputSeedRow = (
      <div className="content ops row settings-row" style={{ paddingTop: 10 }}>
        <Input
          ...
          onChange={event =>
            this.setState({ newSeedPhrase: event.target.value })
          }
        />
        {inputSeedEyeButton}
        <PrimaryButton
          ...
          onClick={() => {
            if (!this.state.newSeedPhrase) {
              ...
              import("ethereum-mnemonic-privatekey-utils").then(pkutils => {
                console.log(pkutils);
                const newPrivateKey = pkutils.getPrivateKeyFromMnemonic(
                  this.state.newSeedPhrase
                );
                changeView("main");
        ...
        */}
        <div style={{ display: "none" }}>
          ...
          <Flex py={3} alignItems="center" justifyContent="space-between">
            ...
            <Checkbox
              onChange={this.updateAdvancedBalance}
              checked={expertMode}
            />
          </Flex>
        ...
        <div>
          ...
          <div
            className="content ops row settings-row"
            style={{ marginBottom: 10 }}
          >
            <a
              href="https://github.com/social-dist0rtion-protocol/planet-a"
              style={{ color: "#FFFFFF" }}
              target="_blank"
            ...
            {/* NOTE: Added presentation of Planet A as "About" for now.
              ...
              */}
            <a
              href="https://docs.google.com/presentation/d/1zjhVQMqVYKIzXUOKR83o5jbo97BRb60XqG6_OwnJOlw/edit"
              style={{ color: "#FFFFFF" }}
              target="_blank"
              rel="noopener noreferrer"
            >
